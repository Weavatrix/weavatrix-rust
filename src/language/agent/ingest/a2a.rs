...
mod tests {
    ...
    #[test]
    fn a2a_cards_are_recognized_by_schema_or_transport_shape() {
        assert!(decode("card.json", "{}", &json!({"name": "x"})).is_none());
        let schema = decode(
            "card.json",
            r#"{"$schema":"https://a2a.dev/card"}"#,
            &json!({"$schema": "https://a2a.dev/card", "name": "desk", "url": "https://x"}),
        )
        .unwrap();
        assert!(schema.symbols.iter().any(|symbol| symbol.name == "desk"));
        let transport = decode(
            "card.json",
            "{}",
            &json!({"preferredTransport": "jsonrpc", "url": "https://x"}),
        );
        assert!(transport.is_some());
        let skills = decode(
            "card.json",
            "{}",
            &json!({"capabilities": {}, "skills": [], "url": "https://x"}),
        );
