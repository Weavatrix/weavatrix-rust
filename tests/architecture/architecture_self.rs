...
#[test]
fn rust_directories_stay_at_ten_source_files() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let mut crowded = Vec::new();
    for relative in ["src", "tests"] {
        collect_directories(&root.join(relative), &mut |directory| {
            ...
            if files > 10 {
                crowded.push(format!(
                    "{} has {files} .rs files",
                    directory.strip_prefix(root).unwrap().display()
                ));
            }
        });
    }
    assert!(crowded.is_empty(), "{}", crowded.join("\n"));
}
...
fn collect_directories(root: &Path, visit: &mut impl FnMut(&Path)) {
    if !root.exists() {
        return;
    }
